...
# Constante de cor fallback caso não esteja no componentes.py
COR_LARANJA_SUAVE = "#FDBA74" 
...
TipoTema = Literal["azul", "verde", "vermelho", "laranja", "cinza"]
...
class DataProcessor:
    ...
    def criar_flag_gpon(df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["GPON_FLAG"] = serie.str.contains("PON", regex=False, na=False).astype(int)
        return df
    ...
    def _ler_csv(arquivo: "UploadedFile") -> pd.DataFrame:
        ...
        for params in tentativas:
            try:
                arquivo.seek(0)
                df = pd.read_csv(arquivo, sep=params["sep"], encoding=params["encoding"], low_memory=False)
                if len(df.columns) > 1:
                    return df
            except Exception:
                continue
    ...
    @staticmethod
    def _aplicar_mascara(df: pd.DataFrame, mascara: Optional[pd.Series]) -> pd.DataFrame:
        if mascara is None: return df
        return df.loc[mascara]

    @staticmethod
    def _contratos_unicos(df: pd.DataFrame, mascara: Optional[pd.Series] = None) -> int:
    ...
    @staticmethod
    def _total_tarefas(df: pd.DataFrame, mascara: Optional[pd.Series] = None) -> int:
    ...
    def _coluna_contem(df: pd.DataFrame, nome_coluna: str, texto: str) -> pd.Series:
        ...
        if coluna is None: return pd.Series(False, index=df.index)
        procurado = Utils.normalizar_texto(texto)
        serie = df[coluna].fillna("").astype(str).map(Utils.normalizar_texto)
        return serie.str.contains(procurado, regex=False, na=False)

    @staticmethod
    def _coluna_igual(df: pd.DataFrame, nome_coluna: str, texto: str) -> pd.Series:
        coluna = Utils.localizar_coluna(df, nome_coluna)
        if coluna is None: return pd.Series(False, index=df.index)
        procurado = Utils.normalizar_texto(texto)
        serie = df[coluna].fillna("").astype(str).map(Utils.normalizar_texto)
    ...
    def _tipo_os_contem(df: pd.DataFrame, texto: str) -> pd.Series:
        ...
        for coluna in Utils.localizar_colunas_tipo_os(df):
            serie = df[coluna].fillna("").astype(str).map(Utils.normalizar_texto)
            mascara = mascara | serie.str.contains(procurado, regex=False, na=False)
        return mascara
    ...
    def calcular_indicadores(df: pd.DataFrame) -> dict[str, int]:
        ...
        mask_rc = DataProcessor._coluna_igual(df, "Tipo de Atividade.1", "Retorno Credenciada")
    ...
    def calcular_rotas(df: pd.DataFrame) -> int:
        ...
        if coluna is None: return 0
        logins = df[coluna].dropna().astype(str).str.strip()
        logins = logins[~logins.isin(["", "nan", "None"])]
        return int(logins.nunique())

    @staticmethod
    def processar_base(nome: str, df: pd.DataFrame, montados: int) -> dict[str, Union[str, int, float]]:
...
class Visualization:
        ...
        with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
            ...
            for idx, coluna in enumerate(ws.columns, start=1):
                ...
                for celula in coluna:
                    if celula.__class__.__name__ == "MergedCell": continue
                    v = str(celula.value) if celula.value is not None else ""
                    if len(v) > maior: maior = len(v)
    ...
    @staticmethod
    def _classe_celula(valor: float, coluna: str, df_bases: Optional[pd.DataFrame] = None) -> str:
        if coluna == "Média OS": return "cel-media-os"
        if coluna == "Média Montados": return "cel-media-mont"
        if df_bases is None or coluna not in df_bases.columns: return "cel-cinza-corp"
        
        valores = pd.to_numeric(df_bases[coluna], errors="coerce").dropna()
        if len(valores) == 0: return "cel-cinza-corp"
        ...
        if posicao == 1: return "cel-heat-verde"
        elif posicao == 2: return "cel-heat-amarelo"
        else: return "cel-heat-vermelho"
    ...
    def renderizar_tabela_html(df: pd.DataFrame) -> None:
        ...
        for i, (_, row) in enumerate(df.iterrows()):
            ...
            for c in df.columns:
                ...
                texto = Utils.fmt_float(v) if col_nome in ("Média OS", "Média Montados") else Utils.fmt_int(v)
    ...
    def grafico_comparativo_medias(df_bases: pd.DataFrame) -> go.Figure:
        fig = go.Figure()
        fig.add_trace(go.Bar(name="Média OS", x=df_bases["BASE"], y=df_bases["Média OS"], marker_color=COR_PRIMARIA, text=[Utils.fmt_float(v) for v in df_bases["Média OS"]], textposition="outside"))
        fig.add_trace(go.Bar(name="Média Montados", x=df_bases["BASE"], y=df_bases["Média Montados"], marker_color=COR_SECUNDARIA, text=[Utils.fmt_float(v) for v in df_bases["Média Montados"]], textposition="outside"))
        fig.update_layout(title="Média de OS por Rota × por Montado", barmode="group", height=380, margin=dict(t=60, b=40, l=20, r=20))
        return fig

    @staticmethod
    def grafico_participacao(df_bases: pd.DataFrame) -> go.Figure:
        color_map: dict[str, str] = {k: v["cor"] for k, v in Config.BASES_CONFIG.items()}
        fig = px.pie(df_bases, values="OS", names="BASE", title="Participação de cada base no total de OS", hole=0.6, color="BASE", color_discrete_map=color_map)
        fig.update_traces(textinfo="percent+label", textfont_size=13, marker=dict(line=dict(color="white", width=3)))
        fig.update_layout(height=380, showlegend=False, margin=dict(t=60, b=20, l=20, r=20))
        return fig
    ...
    def grafico_heatmap(df_bases: pd.DataFrame) -> go.Figure:
        ...
        escala_totale: list[list[Union[float, str]]] = [
            ...
            [0.55, "#FFF7ED"], [0.70, "#FED7AA"], [0.85, "#FDBA74"], [1.00, "#F37C04"],
        ]

        fig = go.Figure(
            data=go.Heatmap(
                z=matriz_norm.to_numpy(), x=list(matriz_norm.columns), y=list(matriz_norm.index),
                text=matriz.to_numpy(), texttemplate="<b>%{text}</b>",
                textfont=dict(family="Inter, sans-serif", size=13, color="#1F2937"),
                colorscale=escala_totale, showscale=True, # type: ignore[arg-type]
                colorbar=dict(title=dict(text="Intensidade", font=dict(family="Inter, sans-serif", size=11, color="#4B5563")), thickness=12, len=0.7, tickfont=dict(family="Inter, sans-serif", size=10, color="#6B7280"), outlinewidth=0),
                hovertemplate="<b>%{y}</b><br>Indicador: <b>%{x}</b><br>Valor: <b>%{text}</b><br>Intensidade: %{z:.0%}<extra></extra>", xgap=4, ygap=4,
            )
        )
        fig.update_layout(
            title=dict(text="<b>Intensidade relativa por indicador</b>", font=dict(family="Manrope, sans-serif", size=16, color="#012869"), x=0.01, xanchor="left"),
            height=340, margin=dict(t=70, b=40, l=90, r=40), paper_bgcolor="white", plot_bgcolor="white",
            xaxis=dict(side="bottom", tickfont=dict(family="Inter, sans-serif", size=12, color="#374151"), showgrid=False, zeroline=False),
            yaxis=dict(tickfont=dict(family="Inter, sans-serif", size=12, color="#012869"), showgrid=False, zeroline=False, autorange="reversed"),
        )
        return fig

    @staticmethod
    def gerar_insights(df_bases: pd.DataFrame, total: dict[str, object]) -> list[tuple[str, TipoInsight]]:
        insights: list[tuple[str, TipoInsight]] = []

        idx_top_os = int(cast(int, df_bases["Média OS"].idxmax()))
        top_os = df_bases.loc[idx_top_os]
        insights.append((f"🥇 **{top_os['BASE']}** possui a maior Média OS: **{Utils.fmt_float(top_os['Média OS'])}** OS por rota.", "ok"))

        idx_low_os = int(cast(int, df_bases["Média OS"].idxmin()))
        low_os = df_bases.loc[idx_low_os]
        if str(low_os["BASE"]) != str(top_os["BASE"]):
            insights.append((f"⬇️ **{low_os['BASE']}** apresenta a menor Média OS: **{Utils.fmt_float(low_os['Média OS'])}** — avaliar produtividade.", "alerta"))
        ...
        insights.append((f"📊 **{base_max['BASE']}** concentra **{Utils.fmt_float(perc, 1)}%** do total de OS ({Utils.fmt_int(valor_base)} de {Utils.fmt_int(total_os)}).", "info"))
        ...
        if total_mont > 0:
            ...
            insights.append((f"🎯 Aproveitamento geral: **{Utils.fmt_float(aprov)}** OS por montado ({Utils.fmt_int(total_mont)} montados no total).", tipo))

        for col in Config.COLUNAS_INDICADORES:
            zerados: list[str] = df_bases[df_bases[col] == 0]["BASE"].astype(str).tolist()
            if zerados and len(zerados) < len(df_bases):
                insights.append((f"⚠️ Indicador **{col}** está zerado em: " + ", ".join(f"**{b}**" for b in zerados), "alerta"))
...
class UI:
    ...
    def reiniciar_painel() -> None:
        chaves_para_limpar = [k for k in list(st.session_state.keys()) if str(k).startswith(("up_", "mont_", "btn_"))]
    ...
    def validar_colunas(nome_base: str, df: pd.DataFrame) -> list[str]:
        ...
        if faltantes:
            render_insight(f"**{nome_base}**: colunas não encontradas: " + ", ".join(f"`{c}`" for c in faltantes), tipo="alerta")
        return faltantes
    ...
    def mostrar_interface() -> None:
        ...
        # Sidebar Profile
        with st.sidebar:
            st.markdown(f"👤 Logado como: **{st.session_state.get('usuario_logado', 'Usuário')}**")
        ...
        render_section_header("folder_open", "Importação das bases", "CSV ou XLSX")
        arquivos = UI._mostrar_uploaders()

        render_section_header("edit_note", "Montados (digitação manual)", "Entrada")
...
[data-testid="stFileUploader"] {
    ...
    border: 2px dashed #CBD5E1; border-radius: 12px; padding: 8px; transition: all 0.25s ease;
}
[data-testid="stFileUploader"]:hover {
    border-color: #012869; background: linear-gradient(180deg, #FFFFFF 0%, #EFF6FF 100%);
}

/* ============ NUMBER INPUTS ============ */
[data-testid="stNumberInput"] input {
    border-radius: 8px !important; border: 1.5px solid #E2E8F0 !important;
    font-weight: 600 !important; font-size: 15px !important; color: #012869 !important;
}
...
.tabela-rota-wrapper {
    ...
    margin: 20px 0 32px 0; overflow: hidden; border: 1px solid #E2E8F0;
}
.tabela-rota { width: 100%; border-collapse: separate; border-spacing: 0; font-family: 'Inter', sans-serif; font-size: 11px; table-layout: fixed; }
.tabela-rota th:first-child, .tabela-rota td:first-child { width: 11%; min-width: 110px; }
.tabela-rota th:not(:first-child), .tabela-rota td:not(:first-child) { width: 7.4%; min-width: 78px; }

.tabela-rota thead th {
    background: linear-gradient(135deg, #F37C04 0%, #E85D04 55%, #C44100 100%); color: white;
    font-weight: 700; text-transform: uppercase; padding: 18px 8px; text-align: center; border: none;
}
.tabela-rota tbody td {
    padding: 16px 8px; text-align: center; border-bottom: 1px solid #F1F5F9;
    font-variant-numeric: tabular-nums; font-weight: 600; font-size: 12px;
}
.tabela-rota tbody tr:hover td { filter: brightness(1.05) saturate(1.08); }

.tabela-rota td.col-base {
    background: linear-gradient(135deg, #012869 0%, #023A9E 55%, #1E5FCC 100%); color: white;
    font-weight: 800; text-align: center; text-transform: uppercase; padding: 18px 10px;
}
.tabela-rota tbody tr.linha-total td {
    background: linear-gradient(135deg, #1F2937 0%, #374151 45%, #4B5563 80%, #6B7280 100%) !important;
    color: white !important; font-weight: 800 !important; border-bottom: none !important;
}
...
/* RANKING CORES */
.cel-heat-verde { background: linear-gradient(180deg, #D1FAE5 0%, #A7F3D0 100%); color: #065F46; font-weight: 700; border-left: 3px solid #10B981; }
.cel-heat-amarelo { background: linear-gradient(180deg, #FEF3C7 0%, #FDE68A 100%); color: #92400E; font-weight: 700; border-left: 3px solid #F59E0B; }
.cel-heat-vermelho { background: linear-gradient(180deg, #FEE2E2 0%, #FECACA 100%); color: #991B1B; font-weight: 700; border-left: 3px solid #EF4444; }
.cel-cinza-corp { background: linear-gradient(180deg, #F3F4F6 0%, #E5E7EB 100%); color: #374151; font-weight: 600; }
.cel-media-os { background: linear-gradient(180deg, #1E5FCC 0%, #023A9E 100%); color: #FFFFFF; font-weight: 800; }
.cel-media-mont { background: linear-gradient(180deg, #F37C04 0%, #C44100 100%); color: #FFFFFF; font-weight: 800; }
</style>
""", unsafe_allow_html=True)
    ...
    def _mostrar_uploaders() -> dict[str, Optional["UploadedFile"]]:
        ...
        for i, base in enumerate(Config.BASES):
            with col_up[i]:
                arquivos[base] = cast("Optional[UploadedFile]", st.file_uploader(f"Base {base}", type=["csv", "xlsx"], key=f"up_{base}_{reset}"))
        return arquivos
    ...
    def _mostrar_inputs_montados() -> dict[str, int]:
        ...
        for i, base in enumerate(Config.BASES):
            with col_mont[i]:
                valor = st.number_input(f"Montados {base}", min_value=0, step=1, value=0, key=f"mont_{base}_{reset}")
                montados_dict[base] = int(valor)
        return montados_dict

    @staticmethod
    def _mostrar_botoes_acao(arquivos: dict[str, Optional["UploadedFile"]], montados_dict: dict[str, int]) -> None:
        st.markdown("<br>", unsafe_allow_html=True)
        col_btn1, col_btn2 = st.columns([3, 1])
        with col_btn1:
            processar = st.button("🚀 Processar bases", type="primary", use_container_width=True, key="btn_processar")
        with col_btn2:
            if st.button("🔄 Reiniciar painel", type="secondary", use_container_width=True, key="btn_reiniciar"):
                UI.reiniciar_painel()
    ...
    @staticmethod
    def _processar_bases(arquivos: dict[str, Optional["UploadedFile"]], montados_dict: dict[str, int]) -> None:
        if not all(arquivos.values()):
            render_insight("Selecione as **três bases** antes de processar.", tipo="critico")
            st.stop()

        with st.spinner("Processando as bases..."):
            bases_dfs: dict[str, Optional[pd.DataFrame]] = {nome: DataProcessor.carregar_arquivo(arq) for nome, arq in arquivos.items()}
            if any(df is None for df in bases_dfs.values()): st.stop()

            bases_validas: dict[str, pd.DataFrame] = {nome: df for nome, df in bases_dfs.items() if df is not None}
            for nome, df in bases_validas.items(): UI.validar_colunas(nome, df)

            resultado: list[dict[str, Union[str, int, float]]] = [DataProcessor.processar_base(nome, df, montados_dict[nome]) for nome, df in bases_validas.items()]
            df_bases = pd.DataFrame(resultado)[Config.ORDEM_COLUNAS]
            df_final = DataProcessor.adicionar_linha_total(df_bases)

        render_insight("Bases processadas com sucesso.", tipo="ok")

        total: dict[str, object] = {str(k): v for k, v in df_final.iloc[-1].to_dict().items()}

        UI._mostrar_kpis_gerais(total)
        UI._mostrar_kpis_por_base(df_bases)

        render_section_header("table_view", "Resultado consolidado", "Detalhamento")
        Visualization.renderizar_tabela_html(df_final)
        
        st.markdown("""
        <div style="display:flex;gap:16px;justify-content:flex-end;align-items:center;padding:8px 4px;font-size:12px;font-family:'Inter',sans-serif;color:#4B5563;font-weight:600;">
        ...
        render_section_header("download", "Exportação", "Excel")
        st.download_button(
            "Baixar resultado em Excel", data=Visualization.gerar_excel(df_final),
            file_name="rota_geral.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            use_container_width=True,
        )

    @staticmethod
    def _mostrar_kpis_gerais(total: dict[str, object]) -> None:
        render_section_header("trending_up", "Indicadores gerais", "Total consolidado")
        k = st.columns(4)
    ...
    def _mostrar_kpis_por_base(df_bases: pd.DataFrame) -> None:
        render_section_header("business", "Desempenho por base", "Comparativo rápido")
    ...
    def _mostrar_graficos(df_bases: pd.DataFrame) -> None:
        render_section_header("analytics", "Análise visual", "Gráficos comparativos")
        g1, g2 = st.columns(2)
        with g1: st.plotly_chart(Visualization.grafico_participacao(df_bases), use_container_width=True)
        with g2: st.plotly_chart(Visualization.grafico_comparativo_medias(df_bases), use_container_width=True)
    ...
    def _mostrar_insights(df_bases: pd.DataFrame, total: dict[str, object]) -> None:
        render_section_header("lightbulb", "Insights automáticos", "Leitura dos dados")
...
def login_screen() -> None:
    st.set_page_config(page_title="Login | Totale", page_icon="🔒", layout="centered")
    aplicar_estilo()
    
    st.markdown("<br><br>", unsafe_allow_html=True)
    render_hero(titulo="Acesso Restrito", subtitulo="Por favor, faça login para acessar a Rota Geral", badge="SEGURANÇA")
    
    with st.form("form_login"):
        st.markdown("#### Informe suas credenciais")
        ...
        if submit:
            if usuario in USUARIOS_PERMITIDOS and USUARIOS_PERMITIDOS[usuario] == senha:
                ...
                st.session_state["usuario_logado"] = usuario.title()
                st.rerun()
            else:
