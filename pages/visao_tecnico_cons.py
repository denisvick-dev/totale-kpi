...
if tec_selecionado and not df_tec.empty:
    ...
    with vc1:
        st.markdown(
            ComponenteVisual.criar_card(
                "Oportunidades", str(t_cons), "azul", "Tentativas", "🗣️"
            ),
    ...
    with vc2:
        st.markdown(
            ComponenteVisual.criar_card(
                "Vendas Fechadas", str(t_prod), "amarelo", "Produtos", "🚀"
            ),
            unsafe_allow_html=True,
        )
    ...
    with vc3:
        st.markdown(
            ComponenteVisual.criar_card(
                "Win Rate", f"{taxa_conversao:.1%}", cor_win, "Conversão", "📈"
            ),
            unsafe_allow_html=True,
        )
    ...
    configs_tabela = {
        "CONSULTIVOS": st.column_config.NumberColumn(
            "🗣️ Oportunidades", help="Total de abordagens"
        ),
