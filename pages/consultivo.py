...
class ComponenteVisual:
    ...
    def criar_card(titulo: str, valor: str, tema: str = "azul", delta: Optional[str] = None) -> str:
        ...
        delta_html = ""
        if delta:
            cor_delta = "#22c55e" if delta.startswith(("+", "▲")) else "#ef4444" if delta.startswith(("-", "▼")) else "#0ea5e9" if "Total" in delta else "#94a3b8"
            simbolo = "▲" if delta.startswith("+") else "▼" if delta.startswith("-") else "◴" if "Total" in delta else "■"
...
class Calculos:
    ...
    def variacao(valor: float, geral: float) -> str:
        if geral == 0 or pd.isna(geral) or abs(valor - geral) < 0.0001: return "Visão Geral"
    ...
    def fator_projecao(df: pd.DataFrame) -> tuple:
        if df.empty or "DATA" not in df.columns or df["DATA"].isna().all(): return 1.0, 0
        hoje = pd.Timestamp.today().normalize()
        if df["DATA"].max().month != hoje.month or df["DATA"].max().year != hoje.year: return 1.0, 0
        ...
        return (dias_uteis_total / dias_decorridos if dias_decorridos > 0 and faltantes > 0 else 1.0), faltantes
...
def preparar_ranking(df: pd.DataFrame, colunas_grupo: list, fator_proj: float = 1.0) -> pd.DataFrame:
    ...
    # Adiciona Posição
    res = res.sort_values("Total Consultivos" if "Total Consultivos" in res.columns else "Total Produtos", ascending=False)
    ...
    # Adiciona os Reais
    if "Total Consultivos" in res.columns: nova_ordem.append("Total Consultivos")
    if "Total Produtos" in res.columns: nova_ordem.append("Total Produtos")
    ...
    for col in ["Mesh", "TV Box", "Virtua"]:
        if col in res.columns: nova_ordem.append(col)
...
t_mesh, t_tv, t_vir = df["Qtde. Mesh"].sum(), df["Qtde. TV"].sum(), df["Qtde. Virtua"].sum()

st.sidebar.header("🎯 Filtros Avançados")
base_sel = st.sidebar.selectbox("Base:", ["Todas"] + sorted(df["Base"].dropna().unique().tolist()))
monitor_opts = ["Todos"] + sorted(df[df["Base"] == base_sel]["Monitor"].dropna().unique().tolist() if base_sel != "Todas" else df["Monitor"].dropna().unique().tolist())
monitor_sel = st.sidebar.selectbox("Monitor:", monitor_opts)

if base_sel != "Todas": df = df[df["Base"] == base_sel]
if monitor_sel != "Todos": df = df[df["Monitor"] == monitor_sel]
...
with c1: st.markdown(ComponenteVisual.criar_card("Total Equipes", f"{eq_total:,.0f}", "azul"), unsafe_allow_html=True)
with c2: st.markdown(ComponenteVisual.criar_card("Equipes Produtivas", f"{eq_produtivas:,.0f}", "verde"), unsafe_allow_html=True)
with c3: st.markdown(ComponenteVisual.criar_card("Técnicos Zerados", f"{eq_total - eq_produtivas:,.0f}", "vermelho"), unsafe_allow_html=True)
with c4: st.markdown(ComponenteVisual.criar_card("Eficiência (Conversão)", f"{eficiencia:.2%}", "roxo"), unsafe_allow_html=True)

st.markdown("#### 📊 Resultado Realizado (Até o momento)")
c5, c6, c7, c8, c9 = st.columns(5)
with c5: st.markdown(ComponenteVisual.criar_card("Tot. Consultivos", f"{f_cons:,.0f}", "azul", Calculos.share(f_cons, t_cons)), unsafe_allow_html=True)
with c6: st.markdown(ComponenteVisual.criar_card("Tot. Produtos", f"{f_prod:,.0f}", "cinza", Calculos.share(f_prod, t_prod)), unsafe_allow_html=True)
with c7: st.markdown(ComponenteVisual.criar_card("Total Mesh", f"{f_mesh:,.0f}", "escuro"), unsafe_allow_html=True)
with c8: st.markdown(ComponenteVisual.criar_card("Total TV Box", f"{f_tv:,.0f}", "escuro"), unsafe_allow_html=True)
with c9: st.markdown(ComponenteVisual.criar_card("Total Virtua", f"{f_vir:,.0f}", "escuro"), unsafe_allow_html=True)

if falt_dias > 0:
    st.markdown(f"#### 🔮 Projeção Fim do Mês <span style='font-size:14px; color:#64748B;'> (Faltam {falt_dias} dias úteis)</span>", unsafe_allow_html=True)
    p1, p2, _ = st.columns([1, 1, 3])
    with p1: st.markdown(ComponenteVisual.criar_card("Proj. Consultivos", f"{int(f_cons * fator_proj):,}", "amarelo", f"+ {int((f_cons * fator_proj) - f_cons)} est."), unsafe_allow_html=True)
    with p2: st.markdown(ComponenteVisual.criar_card("Proj. Produtos", f"{int(f_prod * fator_proj):,}", "amarelo", f"+ {int((f_prod * fator_proj) - f_prod)} est."), unsafe_allow_html=True)
...
with col_tit: st.subheader("👷 Visão Consolidada")
with col_tog: detalhar_tec = st.toggle("Detalhar por Técnico")
...
if colunas_reais: style_df = style_df.set_properties(**{'background-color': '#F8FAFC', 'font-weight': 'bold'}, subset=cast(Any, colunas_reais))
if colunas_proj: style_df = style_df.set_properties(**{'background-color': '#FEF9C3', 'color': '#854D0E', 'font-weight': 'bold'}, subset=cast(Any, colunas_proj))
...
with aba1:
    ...
    with g1:
        if not df_exibir.empty:
            st.plotly_chart(px.bar(df_exibir.head(10), x=grupo[1] if detalhar_tec else "Monitor", y="Total Consultivos", title="Top 10 Consultivos (Real)"), use_container_width=True)
    with g2:
...
if tipo_exp == "CSV":
    c_exp2.download_button("Baixar", df_exibir.to_csv(index=False, encoding='utf-8-sig'), "relatorio.csv", "text/csv")
else:
    out = BytesIO()
    with pd.ExcelWriter(out, engine="openpyxl") as w: df_exibir.to_excel(w, index=False)
    c_exp2.download_button("Baixar", out.getvalue(), "relatorio.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")