...
# =========================================
st.set_page_config(page_title="Dashboard O.S.", page_icon="📊", layout="wide")
...
with c1:
    st.markdown(
        criar_card_metrica(
            "Total O.S. (Geral)", f"{total_geral_os:,}".replace(",", "."), "cinza"
        ),
...
with c2:
    st.markdown(
        criar_card_metrica(
            "Total O.S. (Filtrado)", f"{total_os_filtrado:,}".replace(",", "."), "azul"
        ),
        unsafe_allow_html=True,
    )
with c3:
    st.markdown(
        criar_card_metrica("Projetos Ativos", f"{qtd_projetos}", "laranja"),
        unsafe_allow_html=True,
    )
with c4:
    st.markdown(
        criar_card_metrica("Supervisores Ativos", f"{qtd_supervisores}", "verde"),
        unsafe_allow_html=True,
    )
...
dias_passados = np.busday_count(
    primeiro_dia_np, data_ref_np + np.timedelta64(1, "D"), weekmask="1111110"
)
...
if not df.empty and "Data Agendamento" in df.columns:
    df_tendencia = (
        df.groupby(df["Data Agendamento"].dt.date)["OS"].count().reset_index()
    )
    ...
    fig_linha = px.area(
        ...
        color_discrete_sequence=[
            "#F97316"
        ],  # Laranja: "#F97316", Verde: "#22C55E", Azul: "#0EA5E9"
    )
...
with col_esq:
    ...
    if not df.empty and "Supervisor" in df.columns:
        ...
        qtde_os_sup["Projeção"] = (
            qtde_os_sup["Qtde. de O.S."] + (media_ind * dias_faltantes)
        ).astype(int)

        # APLICANDO A NOVA COLUNA DE FAIXA
        qtde_os_sup["Faixa"] = qtde_os_sup["Qtde. de O.S."].apply(
            definir_faixa_supervisor
        )
...
with col_dir:
    ...
    if not df.empty and "Projeto" in df.columns:
        ...
        qtde_os_proj["Projeção"] = (
            qtde_os_proj["Qtde. de O.S."] + (media_ind * dias_faltantes)
        ).astype(int)
        ...
        with tab_tabela:
            qtde_os_proj["Faixa"] = qtde_os_proj["Qtde. de O.S."].apply(
                definir_faixa_projeto
            )  # Aplicando a função específica para projetos
...
if not df.empty and "Nome Equipe" in df.columns:
    ...
    with col_tec2:
        ...
        fig_top10 = px.bar(
            ...
            range_color=[top10["Qtde. de O.S."].min(), top10["Qtde. de O.S."].max()],
        )
