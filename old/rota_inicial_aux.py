        ...
        try:
            ...
            for col in GestaoAtivos.COLUNAS_PADRAO:
                if col not in df.columns:
                    df[col] = (
                        "" if col != "Valor_Hora" and col != "Custo_Mensal" else 0.0
                    )
...
if not df_bruto.empty:
    ...
    lista_bases = ["Nova Base..."] + sorted(
        df_bruto["Base"].dropna().astype(str).unique().tolist()
    )
    lista_monitores = ["Novo Monitor..."] + sorted(
        df_bruto["Monitor"].dropna().astype(str).unique().tolist()
    )
    ...
    with aba_dash:
        ...
        with c_f1:
            filtro_base = st.selectbox(
                "Filtrar por Base", ["Todas"] + sorted(df_view["Base"].unique())
            )
            if filtro_base != "Todas":
                df_view = df_view[df_view["Base"] == filtro_base]
        with c_f2:
            filtro_sit = st.selectbox(
                "Filtrar por Situação", ["Todas"] + sorted(df_view["Situação"].unique())
            )
            if filtro_sit != "Todas":
        ...
        with c_f3:
            filtro_cont = st.selectbox(
                "Tipo de Contrato",
                ["Todos"] + sorted(df_view["Tipo_Contrato"].unique()),
            )
        ...
        manutencao_pendente = (
            len(df_view[df_view["Proxima_Manutencao"] < str(datetime.now().date())])
        ...
        with kpi3:
            st.markdown(
                criar_card_metrica(
                    "Custo Mensal",
                    GestaoAtivos.formatar_moeda(custo_mensal),
                    "vermelho",
                ),
                unsafe_allow_html=True,
            )
        ...
        with st.form("form_novo_ativo", clear_on_submit=True):
            ...
            with c3:
                nova_situacao = st.selectbox(
                    "Situação *",
                    [s for s in GestaoAtivos.LISTA_SITUACOES if s != "INATIVO"],
                )
                novo_contrato = st.selectbox(
                    "Tipo Contrato *", GestaoAtivos.LISTA_CONTRATOS
                )
                novo_valor_hora = st.number_input(
                    "Valor Hora (R$)", min_value=0.0, step=0.50
                )
                novo_custo = st.number_input(
                    "Custo Mensal Estimado (R$)", min_value=0.0, step=50.00
                )
            ...
            if st.form_submit_button("💾 Salvar Registro", type="primary"):
                ...
                if not novo_re or not novo_login or not novo_nome:
                    ...
                    novo_registro = {
                        ...
                        "Proxima_Manutencao": "",
                        "Observacoes": "",
                        "Ultima_Modificacao": GestaoAtivos.gerar_log_auditoria(
                            usuario_atual, "CRIADO"
                        ),
                    }
                    df_atualizado = pd.concat(
                        [df_bruto, pd.DataFrame([novo_registro])], ignore_index=True
                    )
                    GestaoAtivos.salvar_no_sheets(
                        df_atualizado, "Registro criado com sucesso!"
                    )
            ...
            with col_ed:
                with st.form("form_editar"):
                    ...
                    with ec1:
                        ...
                        st.text_input(
                            "Login", value=dados.get("Login", ""), disabled=True
                        )
                        edit_nome = st.text_input(
                            "Técnico", value=dados.get("Técnico", "")
                        )
                        edit_base = st.text_input("Base", value=dados.get("Base", ""))
                    with ec2:
                        edit_monitor = st.text_input(
                            "Monitor", value=dados.get("Monitor", "")
                        )
                        edit_situacao = st.selectbox(
                            "Situação",
                            GestaoAtivos.LISTA_SITUACOES,
                            index=(
                                GestaoAtivos.LISTA_SITUACOES.index(
                                    dados.get("Situação", "ATIVO")
                                )
                                if dados.get("Situação") in GestaoAtivos.LISTA_SITUACOES
                                else 0
                            ),
                        )
                        edit_contrato = st.selectbox(
                            "Contrato",
                            GestaoAtivos.LISTA_CONTRATOS,
                            index=(
                                GestaoAtivos.LISTA_CONTRATOS.index(
                                    dados.get("Tipo_Contrato", "CLT")
                                )
                                if dados.get("Tipo_Contrato")
                                in GestaoAtivos.LISTA_CONTRATOS
                                else 0
                            ),
                        )
                        edit_custo = st.number_input(
                            "Custo Mensal", value=float(dados.get("Custo_Mensal", 0))
                        )
                    ...
                    if st.form_submit_button("💾 Atualizar Registro", type="primary"):
                        ...
                        df_atualizado.at[idx, "Ultima_Manutencao"] = (
                            edit_ult_man.strftime("%d/%m/%Y") if edit_ult_man else ""
                        )
                        df_atualizado.at[idx, "Proxima_Manutencao"] = (
                            edit_prox_man.strftime("%d/%m/%Y") if edit_prox_man else ""
                        )
                        df_atualizado.at[idx, "Ultima_Modificacao"] = (
                            GestaoAtivos.gerar_log_auditoria(usuario_atual, "EDIÇÃO")
                        )
                        GestaoAtivos.salvar_no_sheets(
                            df_atualizado, "Registro atualizado!"
                        )
            ...
            with col_ex:
                ...
                if st.checkbox(f"Confirmar inativação do RE {dados.get('RE')}"):
                    if st.button(
                        ...
                        df_atualizado.at[idx, "Ultima_Modificacao"] = (
                            GestaoAtivos.gerar_log_auditoria(usuario_atual, "INATIVADO")
                        )
                        GestaoAtivos.salvar_no_sheets(
                            df_atualizado, "Técnico inativado."
                        )
